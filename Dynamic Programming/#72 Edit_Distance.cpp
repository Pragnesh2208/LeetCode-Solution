...
class Solution {
    int solve(int ind1 , int ind2 , string &s1 , string &s2 , vector<vector<int>>&dp) {
        ...
        int insert = solve(ind1 , ind2 + 1 , s1, s2 , dp) + 1;
        int del = solve(ind1 + 1 , ind2 , s1 , s2 , dp) + 1;
        int rep = solve(ind1 + 1 , ind2 + 1 , s1  ,s2 , dp) + 1;

        return dp[ind1][ind2] =  min(insert,min(del , rep));
    }
public:
